...
// The command handler wraps everything in try/catch so a scan fault cannot take pi down —
// which is right, but it leaves the exception with nowhere to land, and the only symptom is a
// notification with a one-line message. This file is that landing place, and it is off by
// default: with the env var unset it never touches the disk.
const LOG_NAME = "pi-environment-monitor.log";
...
/**
 * PI_ENV_MONITOR_DEBUG decides whether and where to log: unset or off returns null; on/1 uses the
...
export function debugLogPath(env: Record<string, string | undefined>, agentDir: string): string | null {
  const raw = env.PI_ENV_MONITOR_DEBUG?.trim();
